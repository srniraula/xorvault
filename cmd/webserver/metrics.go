...
// NewWebOpCtxAt — same as NewWebOpCtx but uses an explicit start time.
// Use this when you capture time.Now() at the very top of a handler
// (before auth, before form parsing) to include all server-side overhead.
func NewWebOpCtxAt(operation, filename, username string, fileSize int64, startTime time.Time) *OperationContext {
	activeUsers.inc(username)
	return &OperationContext{
		operation: operation,
		filename:  filename,
...
func (c *OperationContext) SetUsername(u string)    { c.mu.Lock(); c.username = u; c.mu.Unlock() }
func (c *OperationContext) SetFilename(f string)    { c.mu.Lock(); c.filename = f; c.mu.Unlock() }
