...
// 		return secondaryAddr
// 	}
// 	// Neither node claims primary yet — return "" so caller keeps waiting.
// 	return ""
// }
...
// 	// Threshold reached (or already past it) — probe both masters to find who is active.
// 	// We do NOT reset failureCount here so this probe runs on every subsequent tick
// 	// until we successfully switch.
// 	from := t.activeAddr
// 	newAddr := resolveActiveMaster(t.primaryAddr, t.secondaryAddr)

// 	if newAddr == "" {
// 		// No master has claimed primary yet — the standby is still promoting.
// 		// Keep waiting; probe will fire again on the next heartbeat tick.
...
// 	if newAddr == from {
// 		// Active master recovered — reset failure count and stay.
...
// If the primary master becomes unreachable (6 consecutive failures = 30s), it
// automatically fails over to the secondary master address (if provided).
// addrOverride: if non-empty, use this as myAddr instead of auto-detecting via
// config.GetMyAddr. Set this when the machine has multiple network interfaces
// (e.g. both WiFi and a host-only interface) and auto-detection picks the wrong one.
