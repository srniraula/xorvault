...
// SendHeartbeats sends periodic heartbeats to master server
// Updated to accept localIP, port, and masterAddr
func SendHeartbeats(localIP, port, masterAddr string, logger *log.Logger) {
	ticker := time.NewTicker(5 * time.Second)
	defer ticker.Stop()
	...
	for range ticker.C {
		...
		if err != nil {
			...
			logger.Printf("Heartbeat sent to master: %s -> %s", myAddr, masterAddr)
		}
	}
}
