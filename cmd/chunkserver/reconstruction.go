...
func (c *ChunkServer) reconstructSingleChunk(task *dfspb.ReconstructionTask) error {
	...
	go func() {
		data, checksum, err := c.downloadChunkFromPeer(task.OtherServers[0], task.OtherChunkIds[0], task.Username)
	...
	go func() {
		data, checksum, err := c.downloadChunkFromPeer(task.OtherServers[1], task.OtherChunkIds[1], task.Username)
...
func (c *ChunkServer) downloadChunkFromPeer(serverAddr string, chunkID string, username string) ([]byte, string, error) {
