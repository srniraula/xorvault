...
func main() {
	...
	// Standby is whichever of the two configured addresses is NOT the active one.
	...
	go PerformInventoryCheck(server, *port, tracker, chunkLogger, *myAddrFlag)

	// Start heartbeat goroutine — handles automatic failover via MasterTracker.
	go SendHeartbeats(*port, tracker, chunkLogger, *myAddrFlag)
