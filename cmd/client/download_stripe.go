...
// 	// Count chunks to download (skip empty chunks)
// 	chunksToDownload := 0
...
// 	// If DataChunk2 is empty (not a failure, just doesn't exist) and we have DataChunk1
// 	if stripe.DataChunk2 == nil && stripe.DataChunk1 != nil && stripe.ParityChunk == nil {
...
type StripeDownload struct {
	...
	ParityChunk   []byte // nil if not downloaded
	ChunksOK      int    // Count of successfully downloaded chunks (0-3)
	HasDataChunk2 bool   // true if DataChunk2 exists in this stripe (not just nil due to failure)
}
...
func reconstructMissingChunk(stripe *StripeDownload) error {
	...
	// SPECIAL CASE: Single data chunk stripe (last stripe with odd chunks)
	// If DataChunk2 doesn't exist in this stripe and we have DataChunk1, we're good
	if !stripe.HasDataChunk2 && stripe.DataChunk1 != nil {
		return nil // Valid case: single chunk stripe, no reconstruction needed
	}
