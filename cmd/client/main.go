...
var masterAddr string // Global variable for master address

// main is the entry point - parses command line arguments and calls upload/download
func main() {
	// Define flags
	masterFlag := flag.String("master", "", "master server address (IP:PORT)")
	flag.Parse()

	// Get master address from flag or config file
	if *masterFlag != "" {
		masterAddr = *masterFlag
	} else {
		// Try to read from config file
		masterAddr = readMasterConfig()
		if masterAddr == "" {
			log.Fatal("Master address required. Use -master=IP:PORT or create .master_config file")
		}
	}

	log.Printf("Using master server: %s", masterAddr)
	...
	// Check if user provided correct number of arguments
	if len(args) < 1 {
		log.Fatal("Usage: go run cmd/client/main.go -master=IP:PORT <upload|download|delete|ls> <filename>")
	}
	...
	cmd := args[0] // "upload", "download", "delete", or "ls"

	if cmd == "ls" {
		listFiles(myID)
	} else if len(args) < 2 {
		log.Fatal("Usage: go run cmd/client/main.go -master=IP:PORT <upload|download|delete> <filename>")
	} else {
		file := args[1] // filename to upload/download/delete
...
// readMasterConfig reads master address from .master_config file
func readMasterConfig() string {
	data, err := os.ReadFile(".master_config")
	if err != nil {
		return ""
	}
...
func upload(localPath string, myID int64) {
	...
	// Connect to master server using configured address
