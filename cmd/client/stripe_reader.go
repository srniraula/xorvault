...
// globalParityMs accumulates parity-compute time (nanoseconds stored as int64 for atomicity).
// The upload() function reads and resets this after uploadStripesStreaming returns.
var globalParityNs int64

// parityAccumulator provides thread-safe millisecond accumulation backed by globalParityNs.
type parityAccumulator struct{}

func (parityAccumulator) Add(ms float64) {
	ns := int64(ms * 1e6)
	atomic.AddInt64(&globalParityNs, ns)
}

// DrainParityMs returns accumulated parity time in ms and resets the counter.
func DrainParityMs() float64 {
	ns := atomic.SwapInt64(&globalParityNs, 0)
	return math.Round(float64(ns)/1e4) / 100.0 // round to 0.01 ms
}

var globalParityMs = parityAccumulator{}
...
func streamFileInStripes(filePath string, stripeMap map[int32]*dfspb.StripeMetadata, stripeChan chan<- Stripe, errChan chan<- error) {
	...
	for {
		...
		// Calculate parity for this stripe
		// ── METRICS: time XOR parity computation ────────────────────────────
		pt := newPhaseTimer()
		parity := calculateParity(chunk1, chunk2)
		globalParityMs.Add(pt.ElapsedMs())
		// ────────────────────────────────────────────────────────────────────
