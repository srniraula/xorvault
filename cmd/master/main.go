...
// 	// --- ROLE-BASED STARTUP ---
// 	// PRIMARY:   Try to sync state from the peer. If peer is unreachable, use local WAL
// 	//            state and proceed as primary (peer hasn't promoted itself = safe to be primary).
// 	// SECONDARY: Always start in standby. Never assume primary just because peer is unreachable
// 	//            at startup. The watchdog will promote us only after 30s of silence from primary.
// 	if *role == "secondary" {
// 		// Standby startup: force isPrimary=false regardless of peer reachability.
// 		// Do NOT contact the peer here — we start in standby unconditionally.
// 		// The WatchdogLoop (started below) will promote us if the primary stays silent.
// 		server.isPrimary = false
// 		masterLogger.Printf("Starting as SECONDARY (standby). Will promote only if primary silent for 30s.")
// 	} else if *peerAddr != "" {
// 		// Primary restart: check if peer promoted itself while we were down.
// 		// trySyncStateFromPeer will set isPrimary=false if the peer is the active master.
// 		if err := trySyncStateFromPeer(server, *peerAddr, masterLogger); err != nil {
