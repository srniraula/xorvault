...
func TestUploadStripesStreamingSuccess(t *testing.T) {
	...
	if len(ok) != 3 { t.Fatalf("expected 3 successful chunks, got %d", len(ok)) }
	if !ack.IsEmpty() { t.Fatalf("expected ack queue empty after successful uploads") }
}
...
func TestUploadStripesStreamingFailure(t *testing.T) {
	...
	orig := chunkUploader
	chunkUploader = func(ctx context.Context, serverAddr string, chunkID string, data []byte, clientID int64) error {
		if chunkID == "c2" { return context.Canceled }
		return nil
	}
	defer func(){ chunkUploader = orig }()
