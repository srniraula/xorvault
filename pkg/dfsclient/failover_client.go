...
func (fc *FailoverClient) UploadFile(ctx context.Context, clientID int64, filename string, data io.ReadSeeker, size int64, username string) (int64, TransferStats, error) {
	var result int64
	...
	err := fc.withRetry(func(c *GrpcClient) error {
		...
		var e error
		result, stats, e = c.UploadFile(ctx, clientID, filename, data, size, username)
		return e
	})
	return result, stats, err
}

func (fc *FailoverClient) DownloadFile(ctx context.Context, clientID int64, filename string, destPath string, username string) (TransferStats, error) {
	var stats TransferStats
	err := fc.withRetry(func(c *GrpcClient) error {
		var e error
		stats, e = c.DownloadFile(ctx, clientID, filename, destPath, username)
		return e
	})
	return stats, err
}
