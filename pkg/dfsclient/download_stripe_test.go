...
func TestDownloadStripeAndReconstruct(t *testing.T) {
	...
	chunkDownloader = func(g *GrpcClient, chunkID, serverAddr string, clientID int64, username string, isData1, isData2, isParity bool) DownloadedChunk {
		if isData1 {
			return DownloadedChunk{ChunkID: chunkID, Data: []byte{1, 2, 3}, Success: true, IsData1: true}
		}
		if isParity {
			return DownloadedChunk{ChunkID: chunkID, Data: []byte{1 ^ 0, 2 ^ 0, 3 ^ 0}, Success: true, IsParity: true}
		}
